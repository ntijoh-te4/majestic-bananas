defmodule WTF.GameController do
  require IEx
  ...
  def index(conn) do
    ...
    if is_nil(correct_guesses) do
      IO.puts "Correct guesses is nil"
      correct_guesses = "0"
    end
    ...
    students = Game.get_random_students(correct_student.student_id)
  ...
  def show_alternatives(conn) do
    ...
    send_resp(conn, 200, render("game/default_game",
      correct_student: correct_student,
      students: students,
      name_guessed: name_guessed,
      correct_student_name: correct_student_name,
      counter: counter,
  ...
  def run(conn) do
    ...
    send_resp(conn, 200, render("game/game",
      correct_student: correct_student,
      students: students,
      name_guessed: name_guessed,
      correct_student_name: correct_student_name,
  ...
  def validate_answer(conn, params) do
    ...
    correct_student = conn.private.plug_session["correct_student"]
    correct_student_name = "#{correct_student.student_name}"
    correct_student_id = "#{correct_student.student_id}"
  ...
  def done(conn) do
    send_resp(conn, 200, render("game/done",[]))
  end
